...
const App: React.FC = () => {
  ...
  const handleQuickCustomerSave = async () => {
    ...
    try {
      ...
      setCustomers(prev => [mappedSaved as any, ...prev]);
      ...
      return (
        <div className="space-y-10">
          ...
          <FormSection title="Shipping" icon={Package}>
            ...
            <div className="space-y-1"><Label>State</Label><Input value={selectedItem?.shippingAddress?.state} onChange={localUpdateAddress.bind(null, 'state')} /></div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[800px]">
                <thead className="bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                  <tr><th className="px-8 py-4">Account Holder</th><th className="px-8 py-4">Email Gateway</th><th className="px-8 py-4">Created On</th><th className="px-8 py-4 text-right">Action</th></tr>
                </thead>
                  ...
                  {filteredClaims.map(claim => {
                    ...
                    return (
                      <tr key={claim.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-4 text-sm font-bold text-slate-800">{customer ? `${customer.firstName} ${customer.lastName}` : 'Direct Client'}</td>
           ...
           <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden border border-slate-200">
              ...
              <div className="p-8 space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1"><Label>First Name</Label><Input value={quickCustomer.firstName} onChange={e => setQuickCustomer(prev => ({...prev, firstName: e.target.value}))}/></div>
                    <div className="space-y-1"><Label>Last Name</Label><Input value={quickCustomer.lastName} onChange={e => setQuickCustomer(prev => ({...prev, lastName: e.target.value}))}/></div>
                 </div>
                 <div className="space-y-1"><Label>Email</Label><Input type="email" value={quickCustomer.email} onChange={e => setQuickCustomer(prev => ({...prev, email: e.target.value}))}/></div>
                 <div className="space-y-1"><Label>Phone</Label><Input type="tel" value={quickCustomer.phone} onChange={e => setQuickCustomer(prev => ({...prev, phone: e.target.value}))}/></div>
              </div>
