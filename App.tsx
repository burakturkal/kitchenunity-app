...
import { MOCK_LEADS, MOCK_STORES, MOCK_ORDERS, MOCK_CUSTOMERS, MOCK_CLAIMS, MOCK_INVENTORY } from './services/mockData';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Check for API Key presence to prevent AI-related crashes
  ...
  const handleOpenKeyDialog = async () => {
    if ((window as any).aistudio) {
      ...
      setHasKey(true); // Proceed anyway to avoid race conditions
    }
  };
  ...
  const renderContent = () => {
    ...
    // Safety check for API Key before showing Lead Management (which has AI)
    if (activeTab === 'leads' && hasKey === false) {
      return (
        <div className="flex flex-col items-center justify-center p-20 bg-white rounded-[40px] border border-slate-200 text-center space-y-6">
          <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/10">
            <Key size={32} />
          </div>
          <div>
            <h3 className="text-2xl font-black text-slate-900 tracking-tight">AI Configuration Required</h3>
            <p className="text-slate-500 mt-2 max-w-md mx-auto font-medium">To enable Lead Intelligence and dashboard summaries, please connect your Gemini API project.</p>
          </div>
          <div className="flex flex-col gap-4">
            <button 
              onClick={handleOpenKeyDialog}
              className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all"
            >
              Connect Gemini API
            </button>
            <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-[10px] font-black uppercase text-slate-400 tracking-widest hover:text-blue-500 transition-colors">Billing Documentation</a>
          </div>
        </div>
      );
    }
